...
def _declared_parameter_group(meta: dict[str, Any]) -> str:
    allowed = {"mooring", "rna_balance", "diagonal_hydro", "coupling_disabled"}
    for key in (
        "parameter_group",
        "global_loop_v3_group",
        "global_loop_v2_group",
        "proposal_parameter_group",
    ):
        value = str(meta.get(key) or "")
        if value in allowed:
            return value
    action = meta.get("action") if isinstance(meta.get("action"), dict) else {}
    value = str(action.get("parameter_group") or "")
...
def parameter_group_for_candidate(cand: dict[str, str]) -> str:
    ...
    declared = _declared_parameter_group(meta)
    if declared:
        return declared
...
def parameter_delta_for_candidate(cand: dict[str, str]) -> dict[str, Any]:
    ...
    for key in (
        ...
        "bquad_base",
        "bquad_multiplier",
        "requires_full_sixdof",
        "dof_initial_conditions",
        "proposal_id",
        "proposal_parent_run_id",
        "proposal_target_dof",
