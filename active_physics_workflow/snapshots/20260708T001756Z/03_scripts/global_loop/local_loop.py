...
def _apply_outcome_feedback(
    ...
    refuted = [item for item in entries if _history_is_refuted(item)]
    refuted_counts: dict[str, int] = {}
    for item in refuted:
        group = str(item.get("parameter_group", ""))
        if group:
            refuted_counts[group] = refuted_counts.get(group, 0) + 1
    ...
    for proposal in proposals:
        ...
        if exact and _history_outcome(exact) == "accepted":
            ...
            continue
        if exact and _history_is_refuted(exact):
            adjusted.append(_diagnostic_after_rejection(
                proposal,
                "exact proposal already produced rejected or diagnostic evidence under full 6DOF validation",
                penalty=2.0,
        ...
        if similar_refutations:
            adjusted.append(_diagnostic_after_rejection(
                proposal,
                f"{len(similar_refutations)} similar proposal(s) already produced rejected or diagnostic evidence under full 6DOF validation",
                penalty=min(3.0, 1.5 + 0.75 * len(similar_refutations)),
        ...
        if (
            ...
            and refuted_counts.get("diagonal_hydro", 0) >= 3
        ):
            adjusted.append(_diagnostic_after_rejection(
                proposal,
                "diagonal_hydro has three recent refuted local probes in this oracle epoch; switch to mechanism search or diagnostic review",
                penalty=2.0,
...
def _history_is_refuted(entry: dict[str, Any]) -> bool:
    ...
    for decision in entry.get("step_decisions", []):
        if not isinstance(decision, dict):
            continue
        status = str(decision.get("status", "")).upper()
        hard_violations = decision.get("hard_constraint_violations") or []
        tags = {str(item).lower() for item in decision.get("tags", []) if item}
        reason = str(decision.get("reason", "")).lower()
        if status == "EVIDENCE_ONLY" and hard_violations:
            return True
        if status == "EVIDENCE_ONLY" and (
            "diagnostic_fork" in tags
            or "diagnostic" in tags
            or "incomplete_suite" in tags
            or "diagnostic" in reason
            or "incomplete suite" in reason
        ):
            return True
    return False
...
def _diagnostic_after_rejection(proposal: LocalProposal, reason: str, penalty: float, status: str) -> LocalProposal:
    warned = _memory_warning_proposal(proposal, reason, penalty=penalty, status=status)
    if warned.action.registration_ready and warned.priority <= 0.0:
        diagnostic = _force_diagnostic_proposal(warned, reason)
        return replace(diagnostic, status=f"{status}_diagnostic_only")
    return warned
