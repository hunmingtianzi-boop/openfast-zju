...
def global_step(candidate_id: str, run_live: bool = False) -> dict[str, Any]:
    ...
    suite_rc = 0
    if run_live:
        suite_rc = runner_adapter.run_candidate_suite(candidate_id, list(DOFS), continue_on_failure=True)
    ...
    if not imported:
        if suite_rc != 0:
            raise RuntimeError(f"global candidate suite failed for {candidate_id} with rc={suite_rc} and no registry metrics")
    ...
    next_state = {
        **state,
        ...
        "run_cards_total": len(all_cards),
        "runner_return_code": suite_rc,
        "last_stage": "global_step",
    }
