...
function Get-CurrentEpochId {
    $statePath = Join-Path $SourceRoot "10_global_memory\global_state.json"
    if (Test-Path -LiteralPath $statePath) {
        $state = Get-Content -LiteralPath $statePath -Raw | ConvertFrom-Json
        foreach ($name in @("current_epoch_id", "epoch_id", "current_oracle_id", "oracle_id")) {
    ...
    $manifestPath = Join-Path $SourceRoot "10_global_memory\oracle_manifest.json"
    if (Test-Path -LiteralPath $manifestPath) {
        $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
        if (($manifest.PSObject.Properties.Name -contains "oracle_id") -and $manifest.oracle_id) {
            return [string]$manifest.oracle_id
        }
    }

    throw "Could not determine current oracle epoch from 10_global_memory/global_state.json or oracle_manifest.json."
}
...
$currentEpochId = Get-CurrentEpochId
$currentEpochRel = "10_global_memory\epochs\$currentEpochId"
...
$relPaths = @(
    ...
    "10_global_memory\oracle_manifest.json",
    $currentEpochRel
)
...
$epochRoot = Join-Path $SourceRoot $currentEpochRel
