...
def status() -> dict[str, Any]:
    ...
    state = _without_transient_status_fields(memory.read_global_state())
...
def local_polish(target_dof: str | None = None, top_k: int = 1) -> dict[str, Any]:
    manifest = oracle.ensure_current_manifest()
    state = memory.read_global_state()
    _require_clean_current_best(memory.load_run_cards(), state)
    ...
    next_state = {
        ...
        **harness_mod.summary(harness_assessment),
        "loop_verdict": {"status": "RUNNING", "reason": "local polish recommendation issued", "tags": ["local_polish"]},
    }
    memory.write_global_state(next_state)
    return next_state
...
def materialize_proposal(proposal_id: str) -> dict[str, Any]:
    ...
    if harness_status.get("stop_auto_materialize") or harness_status.get("harness_recommendation") == "STOP_AUTO_MATERIALIZE":
        raise RuntimeError(
            "harness STOP_AUTO_MATERIALIZE is active for the current proposal pool; run diagnostic review or refresh local proposals before materialization"
        )
...
def _proposal_report(pool: Any) -> list[str]:
    ...
    lines = [
        ...
        "",
        "| Rank | Proposal | Target | Action | Ready | Priority | Hypothesis |",
        "|---:|---|---|---|---|---:|---|",
    ]
    for idx, proposal in enumerate(data.get("proposals", []), start=1):
        ...
        lines.append(
            ...
            f"{float(proposal.get('priority', 0.0)):.3f} | {proposal.get('hypothesis', '')} |"
        )
    return lines
...
def _require_clean_current_best(cards: list[dict[str, Any]], state: dict[str, Any]) -> dict[str, Any]:
    selected = select_current_best(cards, state).get("selected_card")
    if not selected or not _is_clean_complete_card(selected):
        raise RuntimeError("no clean complete current_best available; run --offline-import after fixing registry evidence")
    return selected
