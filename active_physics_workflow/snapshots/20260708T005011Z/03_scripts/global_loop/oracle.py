...
def build_manifest(cfg: dict[str, Any] | None = None) -> OracleManifest:
    ...
    payload = {
        ...
        "starting_model_tree_hash": config.cfgmod.sha256_tree(
            model_root,
            exclude_suffixes=ORACLE_MODEL_EXCLUDE_SUFFIXES,
        ),
        "target_source": "active_curated_processed_reference",
    }
    truth_payload = _truth_payload(payload)
    semantic_payload = _semantic_truth_payload(payload)
    compatible = _compatible_existing_manifest(semantic_payload)
    if compatible and compatible.get("oracle_id"):
        oracle_id = str(compatible["oracle_id"])
        created_at = str(compatible.get("created_at") or config.utc_now())
    elif existing and _truth_payload(existing) == truth_payload and existing.get("oracle_id"):
        oracle_id = str(existing["oracle_id"])
        created_at = str(existing.get("created_at") or config.utc_now())
    else:
        oracle_id = "oracle_" + _sha256_json(semantic_payload)[:16]
...
def _semantic_truth_payload(data: dict[str, Any]) -> dict[str, Any]:
    payload = _truth_payload(data)
    payload.pop("active_paths", None)
    return payload


def _compatible_existing_manifest(semantic_payload: dict[str, Any]) -> dict[str, Any]:
    candidates: list[dict[str, Any]] = []
    top = read_manifest()
    if top and _semantic_truth_payload(top) == semantic_payload and top.get("oracle_id"):
        candidates.append(top)
    root = epochs_root()
    if root.exists():
        for path in root.glob(f"*/{ORACLE_MANIFEST_NAME}"):
            try:
                item = read_manifest(path)
            except (OSError, json.JSONDecodeError):
                continue
            if item and _semantic_truth_payload(item) == semantic_payload and item.get("oracle_id"):
                candidates.append(item)
    if not candidates:
        return {}
    candidates.sort(
        key=lambda item: (
            _epoch_runcard_count(str(item.get("oracle_id", ""))),
...
def _epoch_runcard_count(oracle_id: str) -> int:
    if not oracle_id:
        return 0
    path = epoch_runcards_dir(oracle_id)
    if not path.exists():
        return 0
