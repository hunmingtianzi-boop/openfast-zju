...
def main() -> None:
    ...
    out = c.RUN_ROOT / "selected_inertia_grid_roll_pitch.csv"
    ...
    for rf, pf in pairs:
        if (rf, pf) in done:
            continue
        model_id = f"sel_inertia_R{str(rf).replace('.', 'p')}_P{str(pf).replace('.', 'p')}"
        cand = c.Candidate(
            model_id,
            base_clin,
            inertia_factors={"PtfmRIner": rf, "PtfmPIner": pf},
            basis="selected roll/pitch inertia period closure",
        )
